...
class Cf7_Send_Wa {
	...
	private function define_public_hooks() {
		...
		if ( in_array( 'woocommerce/woocommerce.php', apply_filters( 'active_plugins', get_option( 'active_plugins' ) ) ) ) {
            ...
            // single product button hook
            $button_hook = get_option( 'cf7sendwa_single_button_hook', 'woocommerce_after_add_to_cart_button' );
            $types = array( 'variable', 'simple' );
            foreach( $types as $ptype ) {
                add_action( 'woocommerce_before_single_product_summary', function() use($button_hook, $ptype, $plugin_public) {
                    if( $button_hook == 'replace' ) {
                        remove_action( 'woocommerce_' . $ptype . '_add_to_cart', 'woocommerce_' . $ptype . '_add_to_cart', 30 );
                        $button_hook = 'woocommerce_single_product_summary';
                    }
